...
fun workspaceSymbols(query: String, sp: SourceFileRepository): List<WorkspaceSymbol> =
        doWorkspaceSymbols(sp)
                .filter { containsCharactersInOrder(it.name!!, query, false) }
                .mapNotNull(::workspaceSymbol)
                .toList()

private fun doWorkspaceSymbols(sp: SourceFileRepository): Sequence<KtNamedDeclaration> =
        sp.allFiles().asSequence().flatMap(::fileSymbols)
